...
# Create a task
just trellis create --type calendaritem --id 'calendaritem:my-task' \
...
# Delete a node
just trellis delete calendaritem:my-task --agent-id cleanup-bot
```
...
const tasks = await client.query('FIND calendaritem AS ?e WHERE ?e.type = "task"')
await client.createNode('calendaritem:new-task', 'calendaritem', {
  type: 'task',
