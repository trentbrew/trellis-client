...
const callback_get = defineEventHandler(async (event) => {
  ...
  const successRedirect = returnTo ? `${returnTo}${returnTo.includes("?") ? "&" : "?"}connected=google-calendar` : "/settings/integrations?connected=google-calendar";
  ...
  try {
    ...
    console.error("[gcal/callback] Token exchange failed:", (err == null ? void 0 : err.data) || err);
