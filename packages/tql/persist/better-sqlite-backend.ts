/**
 * BetterSqlite3 Kernel Backend
 *
 * Node-compatible SQLite backend using `better-sqlite3`.
...
export type BetterSqliteBackendOptions = {
  filename: string;
};

export class BetterSqliteBackend implements KernelBackend {
  private db: any;
  ...
  readAll(): KernelOp[] {
    ...
    return rows.map((r: { payload: string }) =>
      decodeOp(JSON.parse(r.payload)),
    );
  }
