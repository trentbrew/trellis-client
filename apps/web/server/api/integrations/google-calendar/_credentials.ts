/**
 * Shared Google Calendar credentials helper.
 *
 * Mirrors `../gmail/_credentials.ts` — loads stored OAuth credentials for a
 * connection, refreshes the access token when expired, and persists refreshed
 * tokens back to TQL.
 */
...
export async function refreshAccessToken(
  ...
  const tokenData: { access_token: string; expires_in: number } = await $fetch(
...
export async function getValidAccessToken(connectionId: string): Promise<string> {
  ...
  if (creds.expiresAt < Date.now() + REFRESH_BUFFER_MS) {
    ...
    try {
      ...
      console.error('[gcal/_credentials] Token refresh failed:', err?.data || err)
