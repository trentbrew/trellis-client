import type { Entity, Priority, Urgency, FormulaField } from '~/types/entity'
 ...
 */
const computeDefaultPriority = (item: Entity): Priority => {
  const now = new Date()
  const start = parseYmdLocal(item.startDate)
 ...
 */
const computeDefaultUrgency = (item: Entity): Urgency => {
  const now = new Date()
  const start = parseYmdLocal(item.startDate)
 ...
 */
const evaluateFormula = (expression: string, item: Entity): unknown => {
  try {
 ...
 * on Entity instances. Values are reactive and update when the item changes.
 *
 ...
 */
export const useEntityFormulas = () => {
  /**
   ...
   */
  const getEffectivePriority = (item: Entity): Priority => {
   ...
   */
  const getEffectiveUrgency = (item: Entity): Urgency => {
   ...
   */
  const evaluateFormulas = (item: Entity): FormulaField[] => {
   ...
   */
  const applyFormulas = (item: Entity): void => {
