...
export function useScrollspy(options: UseScrollspyOptions = {}) {
  ...
  function queryAnchors() {
    anchors.value = root.value ? Array.from(root.value.querySelectorAll(`[data-${dataAttribute}-anchor]`)) : []
  }
  ...
  const onScroll = throttleTime ? useThrottleFn(handleScroll, throttleTime, true, false) : handleScroll

  // Rebind when scroller changes
  useEventListener(scroller, 'scroll', onScroll, { passive: true })

  // --- Click to scroll ---
  function scrollTo(idOrEl: string | HTMLElement, behavior: ScrollBehavior = smooth ? 'smooth' : 'auto') {
    const se = scroller.value
  ...
  onMounted(() => {
    ...
    if (raw) {
      ...
      const targetAnchor = root.value?.querySelector(`[data-${dataAttribute}-anchor="${safe}"]`) as HTMLElement | null
