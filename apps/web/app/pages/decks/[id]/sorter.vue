<script setup lang="ts">
  import DeckSorterShell from '~/components/deck/DeckSorterShell.vue'
  ...
  useHead({ title: () => `${deckSlug.value} · Sorter | Deck` })
</script>

<template>
  <Page variant="canvas" :fill-height="true" hide-sidebar>
    <DeckSorterShell v-if="deckId" :deck-id="deckId" :initial-slide-index="slideIndex" class="h-full min-h-0" />
  </Page>
</template>
