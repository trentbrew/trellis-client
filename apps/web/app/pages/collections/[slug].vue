<script setup lang="ts">
  ...
  import { createDefaultProjections, normalizeProjections, buildProjectionTypeOptions } from '~/lib/projections'
  ...
  const PRIMARY_PROJECTION_TYPES: Array<Projection['type']> = ['table', 'kanban', 'calendar', 'graph', 'list']

  const primaryProjections = computed(() => {
    return projections.value
      .filter((p) => PRIMARY_PROJECTION_TYPES.includes(p.type))
      .slice()
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
  })

  const advancedProjections = computed(() => {
    return projections.value
      .filter((p) => !PRIMARY_PROJECTION_TYPES.includes(p.type))
      .slice()
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
  ...
  const newViewOptions = computed(() => {
    return buildProjectionTypeOptions(schema.value, ['table', 'kanban', 'calendar', 'list', 'graph'], {
        ...
        <div class="relative px-6 py-3">
          <div class="flex items-center justify-between gap-0">
            <UiDropdownMenu v-model:open="isProjectionMenuOpen">
              ...
              <UiDropdownMenuContent align="start" class="w-64">
                <UiDropdownMenuLabel>Views</UiDropdownMenuLabel>
                <UiDropdownMenuSeparator />
                <UiDropdownMenuItem
                  v-for="proj in primaryProjections"
                  :key="proj.id"
                  class="gap-2"
                  @click="activeProjection = proj.id">
                  <Icon :name="proj.icon || 'lucide:circle'" class="h-4 w-4" />
                  <span class="flex-1">{{ proj.name }}</span>
                  <Icon v-if="proj.id === activeProjection" name="lucide:check" class="h-4 w-4 text-primary" />
                </UiDropdownMenuItem>

                <template v-if="advancedProjections.length">
                  <UiDropdownMenuSeparator />
                  <UiDropdownMenuLabel>Advanced</UiDropdownMenuLabel>
                  <UiDropdownMenuSeparator />
                  <UiDropdownMenuItem
                    v-for="proj in advancedProjections"
                    :key="proj.id"
                    class="gap-2"
                    @click="activeProjection = proj.id">
                    <Icon :name="proj.icon || 'lucide:circle'" class="h-4 w-4" />
                ...
                <UiDropdownMenuSeparator />
                <UiDropdownMenuLabel>New view</UiDropdownMenuLabel>
                <UiDropdownMenuSeparator />
                <UiTooltipProvider v-for="opt in newViewOptions" :key="opt.type">
                  <UiTooltip :disabled="!opt.disabled">
                    <UiTooltipTrigger as-child>
                      <UiDropdownMenuItem
                        class="gap-2"
                        :disabled="opt.disabled"
                        @click="!opt.disabled && _addProjection(opt.type, opt.name)">
                        <Icon :name="opt.icon" class="h-4 w-4" />
                        <span class="flex-1">{{ opt.name }}</span>
                      </UiDropdownMenuItem>
                    </UiTooltipTrigger>
                    <UiTooltipContent v-if="opt.tooltip" side="left">
                      {{ opt.tooltip }}
                    </UiTooltipContent>
                  </UiTooltip>
                </UiTooltipProvider>
      ...
      <div
        ...
        <template v-for="proj in projections" :key="proj.id">
          ...
          <UiTabsContent v-else-if="proj.type === 'graph'" :value="proj.id" class="h-full mt-0">
            <div class="flex h-full items-center justify-center p-8">
              <div class="max-w-md text-center space-y-3">
                <Icon name="lucide:network" class="h-10 w-10 text-muted-foreground mx-auto" />
                <h3 class="text-lg font-semibold">Graph</h3>
                <p class="text-sm text-muted-foreground">
                  This projection will visualize relationships in your data. It’s available now, with rendering coming
                  next.
                </p>
              </div>
            </div>
          ...
          <UiTabsContent v-else-if="proj.type === 'list'" :value="proj.id" class="h-full mt-0">
            <div class="flex h-full items-center justify-center p-8">
              <div class="max-w-md text-center space-y-3">
                <Icon name="lucide:list" class="h-10 w-10 text-muted-foreground mx-auto" />
                <h3 class="text-lg font-semibold">List</h3>
                <p class="text-sm text-muted-foreground">
                  A lightweight list rendering of the collection. This is available for every collection; layout rules
                  coming soon.
                </p>
              </div>
            </div>
