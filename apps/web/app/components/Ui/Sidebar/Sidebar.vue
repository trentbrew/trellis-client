<script lang="ts">
  ...
  export const sideBarStyles = tv({
    slots: {
      collapsible: 'flex h-full w-(--sidebar-width) flex-col bg-sidebar-background text-sidebar-foreground',
    ...
    variants: {
      ...
      variant: {
        ...
        floating: {
          sideBarWrapper: 'group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]',
          sideBarWrapper2: 'p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]',
        },
        inset: {
          sideBarWrapper: 'group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]',
          sideBarWrapper2: 'p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]',
...
<template>
  ...
  <UiSheet v-else-if="isMobile" :open="openMobile" v-bind="$attrs" @update:open="setOpenMobile">
    ...
    >
      <VisuallyHidden>
        ...
        <UiSheetDescription>This is the mobile sidebar. You can use this to navigate the site.</UiSheetDescription>
      </VisuallyHidden>
  ...
  >
    ...
    <div :class="sideBarStyles().sideBarWrapper2({ collapsible, side, variant, class: props.class })" v-bind="$attrs">
