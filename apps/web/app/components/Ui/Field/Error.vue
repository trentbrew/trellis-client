...
<template>
  <Primitive :as :as-child role="alert" data-slot="field-error" :class="fieldErrorStyles({ class: props.class })">
    <slot>
        ...
        <template v-if="Array.isArray(errors)">
          <ul class="ml-4 flex list-disc flex-col gap-1">
            <template v-for="(item, i) in errors" :key="i">
              <li>
                {{ typeof item === 'object' && item !== null && 'message' in item ? item.message : item }}
              </li>
            </template>
          </ul>
        </template>
