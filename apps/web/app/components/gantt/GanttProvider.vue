<script setup lang="ts">
  ...
  const todayOffscreen = ref(false)
  const todayOffscreenDirection = ref<'left' | 'right' | null>(null)
  ...
  const computeInitialStartDate = () => {
    if (props.range === 'hourly') {
      return props.startDate ? startOfDay(props.startDate) : startOfDay(new Date())
    }
    return props.startDate ?? new Date()
  }

  const timelineStartDate = ref<Date>(computeInitialStartDate())

  // Update timelineStartDate when startDate prop changes
  watch(
    () => props.startDate,
    (newStartDate) => {
      if (newStartDate && props.range === 'hourly') {
        timelineStartDate.value = startOfDay(newStartDate)
  ...
  const setTodayOffscreen = (offscreen: boolean, direction: 'left' | 'right' | null) => {
    todayOffscreen.value = offscreen
    todayOffscreenDirection.value = direction
  }
  ...
  const context = shallowReactive<GanttContextProps>({
    ...
    scrollToDate,
    scrollToToday,
    todayOffscreen: todayOffscreen.value,
    todayOffscreenDirection: todayOffscreenDirection.value,
    setTodayOffscreen,
  ...
  watch(todayOffscreen, (val) => {
    context.todayOffscreen = val
  })

  watch(todayOffscreenDirection, (val) => {
    context.todayOffscreenDirection = val
  })
  ...
  const handleScrollRaf = () => {
    ...
    rafId = window.requestAnimationFrame(() => {
      rafId = null
      scrollX.value = scrollRef.value?.scrollLeft ?? 0
    })
  }
  ...
  onMounted(() => {
    if (scrollRef.value) {
      ...
      scrollX.value = scrollRef.value.scrollLeft
      scrollRef.value.addEventListener('scroll', handleScrollRaf, { passive: true })
  ...
  onUnmounted(() => {
    if (scrollRef.value) {
      scrollRef.value.removeEventListener('scroll', handleScrollRaf)
