<script setup lang="ts">
  import SlideFilmstrip from './SlideFilmstrip.vue'
...
<template>
  <div class="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-border bg-card" data-vantage="sorter">
    ...
    <template v-else>
      ...
      <div class="flex min-h-0 flex-1 flex-col md:flex-row">
        <div class="flex min-w-0 flex-1 flex-col p-4">
          <div class="mx-auto aspect-video w-full max-w-lg overflow-hidden rounded-md border border-border bg-[#0d0d11]">
            <SlideCanvas
              v-if="activeSlide"
              :slide="activeSlide"
              ...
              read-only
              :tab-id="`sorter-panel-${activeIndex}`"
              :labelled-by="`filmstrip-tab-${activeIndex}`"
            />
          </div>
          <SpeakerNotesStrip
            v-if="activeSlide"
