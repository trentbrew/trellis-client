<script lang="ts" setup>
  /**
   * ContainerDialogShell — Dialog chrome for container-class entities.
   *
   * Optimized for collection/grouping entities (project, folder, collection, workspace, tag-group).
   * Differences from TemporalDialogShell:
   *  - Item count badge in header
   *  - Wider default to accommodate nested item lists
   *  - Content area typically holds a list/table of child entities
   */
...
<template>
  <UiDialog :open="open" @update:open="emit('update:open', $event)">
    <UiDialogContent
      ...
      <UiDialogTitle class="sr-only">{{ dialogTitle || title || 'Collection' }}</UiDialogTitle>
      <UiDialogDescription class="sr-only">{{ dialogDescription || 'Container details.' }}</UiDialogDescription>

      <!-- Header — collection-oriented with item count -->
        ...
        <div :class="isStacked && parentTitle ? 'px-4 pt-2 pb-3' : 'px-4 pt-4 pb-3'">
          <div class="flex items-center justify-between gap-3 mb-3">
            <div class="flex items-center gap-2 min-w-0">
              ...
              <span v-if="itemCount > 0" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-muted text-muted-foreground">
          ...
          <input
            ...
            :value="title"
            type="text"
            :placeholder="titlePlaceholder || 'Collection name...'"
          ...
          <div class="mt-1 px-1">
            <UiRichTextEditor v-if="!isViewMode" :model-value="description" placeholder="Describe this collection..." seamless @update:model-value="emit('update:description', $event)" />
      ...
      <!-- Content Area — wide, list-oriented -->
