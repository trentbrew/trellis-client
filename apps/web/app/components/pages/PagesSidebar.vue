<script setup lang="ts">
  ...
  function isActive(id: string) { return activePageId.value === id }
  ...
  async function commitRename() {
    ...
    if (!id || !title) return
    try { await renamePage(id, title) }
    catch { ;($toast as any)?.error('Failed to rename page') }
  }
  ...
  async function commitRenameFolder() {
    ...
    try { await renameFolder(oldName, newName) }
    catch { ;($toast as any)?.error('Failed to rename folder') }
  }
  ...
  async function commitNewFolder() {
    ...
    try { await moveToFolder(pageId, name) }
    catch { ;($toast as any)?.error('Failed to create folder') }
    finally { creatingFolderForId.value = false; newFolderForId.value = null; newFolderForIdName.value = '' }
  }

  async function handleMoveToFolder(pageId: string, folder: string | null) {
    try { await moveToFolder(pageId, folder) }
    catch { ;($toast as any)?.error('Failed to move page') }
  }

  async function handleDeleteFolder(name: string) {
    try { await deleteFolder(name) }
    catch { ;($toast as any)?.error('Failed to delete folder') }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      await deletePage(id)
      if (isActive(id)) navigateTo('/pages')
    } catch {
...
<template>
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <UiAlertDialog :open="!!deleteTargetId" @update:open="(v) => { if (!v) deleteTargetId = null }">
    ...
    <div class="shrink-0">
      ...
      <div v-else>
        ...
        <nav class="px-2 space-y-0.5" data-pages-sortable data-folder-name="">
              ...
              <div class="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover/pg:opacity-100 transition-opacity z-10">
                <UiDropdownMenu>
                  <UiDropdownMenuTrigger as-child>
                    <button type="button" class="h-5 w-5 flex items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-muted/80" aria-label="Page options" @click.prevent.stop>
          ...
          <div v-if="!ungroupedPages.length && !pagesByFolder.length && !loading" class="px-2 py-1.5 text-xs text-muted-foreground/60 italic">
            No pages yet
          </div>
        </nav>
        ...
        <Transition name="slide-down">
          <div v-if="newFolderForId" class="px-3 py-1.5">
            ...
            <div class="flex items-center gap-1.5 rounded-lg border border-border bg-background px-2 py-1">
              ...
              />
              <button :disabled="!newFolderForIdName.trim() || creatingFolderForId" class="text-xs text-primary disabled:opacity-40 hover:text-primary/80 transition-colors font-medium" @click="commitNewFolder">
                Create
              </button>
            </div>
          </div>
          ...
          <div v-for="group in pagesByFolder" :key="group.folder" data-folder-row>
            ...
            <div class="group/folder relative">
              ...
              >
                ...
                >
                  <Icon name="lucide:chevron-right" class="h-3 w-3 transition-transform duration-150 shrink-0" :class="isFolderOpen(group.folder) ? 'rotate-90' : ''" />
                  <Icon name="lucide:folder" class="h-3 w-3 shrink-0" />
                  <span class="flex-1 text-left truncate">{{ group.folder }}</span>
                </button>
                <!-- Folder actions -->
                <div class="flex items-center gap-0.5 opacity-0 group-hover/folder:opacity-100 transition-opacity shrink-0">
                  <UiDropdownMenu>
                    <UiDropdownMenuTrigger as-child>
                      <button type="button" class="h-4 w-4 flex items-center justify-center rounded text-muted-foreground/60 hover:text-foreground hover:bg-muted" @click.stop>
                    ...
                    <UiDropdownMenuContent align="end" :side-offset="4" class="w-44">
                      <UiDropdownMenuItem @click="startRenameFolder(group.folder)">
                        <Icon name="lucide:pencil" class="mr-2 h-4 w-4" />Rename folder
                      </UiDropdownMenuItem>
                      <UiDropdownMenuItem @click="handleCreate(group.folder)">
                        <Icon name="lucide:plus" class="mr-2 h-4 w-4" />Add page
                      </UiDropdownMenuItem>
                      <UiDropdownMenuSeparator />
                      <UiDropdownMenuItem class="text-destructive focus:text-destructive" @click="handleDeleteFolder(group.folder)">
                        <Icon name="lucide:folder-minus" class="mr-2 h-4 w-4" />Delete folder
                      </UiDropdownMenuItem>
                    </UiDropdownMenuContent>
                  </UiDropdownMenu>
                  <button
              ...
              <div v-if="isFolderOpen(group.folder)" class="relative">
                ...
                >
                  <template v-for="page in group.pages" :key="page.id">
                    ...
                    >
                      <!-- <span
                        class="absolute inset-y-0 left-0 w-8 z-10 flex items-center justify-center opacity-0 group-hover/pg:opacity-100 transition-opacity"
                      >
                        <Icon name="lucide:grip-vertical" class="h-3 w-3 text-muted-foreground/50" />
                      </span> -->
                      <NuxtLink
