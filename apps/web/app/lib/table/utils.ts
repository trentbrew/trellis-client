...
function collectCells(
  ...
  if (resolvedIndex < 0 || resolvedIndex >= maxIndex) return EMPTY_CELLS_RESULT
  ...
  try {
    ...
    if (orientation === 'row') {
      if (newIndex >= 0 && newIndex < map.height) {
        const startCellPos = table.start + map.positionAt(newIndex, 0, table.node)
        const endCellPos = table.start + map.positionAt(newIndex, map.width - 1, table.node)
      ...
      if (newIndex >= 0 && newIndex < map.width) {
        const startCellPos = table.start + map.positionAt(0, newIndex, table.node)
        const endCellPos = table.start + map.positionAt(map.height - 1, newIndex, table.node)
    ...
    if (dispatch) {
      ...
      if (state.selection instanceof CellSelection) {
        state.selection.forEachCell((node, pos) => {
          const needsUpdate = Object.entries(attrs).some(([k, val]) => node.attrs[k] !== val)
          if (needsUpdate) {
        ...
        const needsUpdate = Object.entries(attrs).some(
          ([k, val]) => $cell.nodeAfter!.attrs[k] !== val,
        )
        if (needsUpdate) {
...
export function selectLastCell(
  ...
  if (!cellPos && cellPos !== 0) return false
...
export function selectCellAt({
  ...
  if (!isWithinBounds(row, col, found.map)) return false
...
export function getIndexCoordinates({
  ...
  if (orientation === 'row' && index >= map.height) return null
  if (orientation === 'column' && index >= map.width) return null
...
export function selectCellsByCoords(
  ...
  if (cleanedCoords.length === 0) return undefined
  ...
  if (headPosition === null) return undefined
