...
import { createTableDragImage } from '~/lib/table/create-drag-image'
...
class TableHandleView implements PluginView {
  ...
  private _handleMouseMoveNow(event: MouseEvent) {
    ...
    if (around.type === 'wrapper') {
      const below =
        event.clientY >= tableRect.bottom - 1 && event.clientY < tableRect.bottom + 20
      const right =
        event.clientX >= tableRect.right - 1 && event.clientX < tableRect.right + 20
  ...
  update(view: EditorView): void {
    ...
    if (!tableBody) {
      throw new Error("Table block does not contain a 'tbody' HTML element.")
    }
