...
const TRELLIS_PATH = resolve(TQL_DIR, 'docs.trellis.jsonld');
...
export function loadTrellis(): DocModule[] {
  if (!existsSync(TRELLIS_PATH)) {
    console.error('[Living Docs] No .tql/docs.trellis.jsonld found.');
    return [];
  }
...
export function buildDocument(module: DocModule, kernel: TrellisKernel, existingContent: string): string {
  ...
  md += `---\n*Auto-generated by [TQL Living Docs](../.tql/docs.trellis.jsonld) at ${now}*\n`;
  return md;
}
...
async function main() {
  ...
  try {
    if (existsSync(resolve(TQL_DIR, 'ops.jsonl'))) {
      kernel = createKernel();
    }
  } catch (err) {
